...
int main(int argc, char** argv){
    ...
    if(!SFENParse(initSfen)) renderBoard();
    // puts("-----------------------");
    // if(!SFENParse(nextSfen)) renderBoard();
    // scrollKifu();
    while (true){
    userInput();
    // printf("%d",validMove(input.init, input.final, *getPieceBycoord(input.init)));
    if(validMove(input.init, input.final, *getPieceBycoord(input.init))){
